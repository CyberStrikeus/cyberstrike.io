...
<div className="border-2 border-dashed border-gray-400 dark:border-gray-600 rounded-lg p-8 my-6 text-center bg-gray-100 dark:bg-gray-800">
  <p className="text-gray-500 dark:text-gray-400 font-mono text-sm">📊 DIAGRAM: cicd-workflow.mmd</p>
  ...
  scan:
    ...
    steps:
      ...
      - name: Install Cyberstrike
        run: npm install -g @cyberstrike-io/cli
...
pipeline {
    ...
    stages {
        stage('Install') {
            steps {
                sh 'npm install -g @cyberstrike-io/cli'
            }
        }
...
jobs:
  security-scan:
    ...
    steps:
      ...
      - run:
          name: Install Cyberstrike
          command: npm install -g @cyberstrike-io/cli
      - run:
          name: Run Security Scan
          command: |
