...
We want to make it easy for you to contribute to CyberStrike. Here are the most common types of changes that get merged:

- Bug fixes
- New security tool integrations (MCP Kali, Bash wrappers)
- New WSTG/OWASP knowledge base checklists
- Improvements to pentest agent prompts and methodology
- Support for new AI providers
- Improvements to the TUI and terminal experience
...
- [`help wanted`](https://github.com/CyberStrikeus/cyberstrike.io/issues?q=is%3Aissue%20state%3Aopen%20label%3Ahelp-wanted)
- [`good first issue`](https://github.com/CyberStrikeus/cyberstrike.io/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22good%20first%20issue%22)
- [`bug`](https://github.com/CyberStrikeus/cyberstrike.io/issues?q=is%3Aissue%20state%3Aopen%20label%3Abug)
- [`perf`](https://github.com/CyberStrikeus/cyberstrike.io/issues?q=is%3Aopen%20is%3Aissue%20label%3A%22perf%22)
...
CyberStrike includes 4 pentest agents. You can run a specific one during development:

```bash
bun dev --agent web-application    # OWASP/WSTG web testing
bun dev --agent cloud-security     # AWS/Azure/GCP audit
bun dev --agent internal-network   # AD & network pentest
bun dev --agent bug-hunter         # Bug bounty recon
```
...
To run CyberStrike in the root of the repo itself:
...
- `packages/cyberstrike`: Core CLI, business logic & server
- `packages/cyberstrike/src/agent/prompt/`: Pentest agent prompt files
- `packages/cyberstrike/src/cli/cmd/tui/`: TUI code (SolidJS + [@cyberstrike/tui-solid](./packages/tui-solid))
- `packages/mcp-kali`: MCP server for 100+ Kali Linux tools
...
- `knowledge/`: WSTG security checklists (120+ test cases)
...
CyberStrike's effectiveness depends on its knowledge base. You can contribute by:

- Adding new WSTG test case checklists to `knowledge/web-application/`
- Improving existing agent prompts in `packages/cyberstrike/src/agent/prompt/`
- Adding tool wrappers for security tools (nmap, nuclei, sqlmap, etc.)
- Extending MCP Kali tool definitions in `packages/mcp-kali/`

When contributing security content, ensure methodology references are accurate and follow established frameworks (OWASP, PTES, NIST).
...
- `fix(mcp-kali):` bug fix in the MCP Kali package
...
- `feat(agent): add API fuzzing methodology to web-application agent`
- `feat(mcp-kali): add gobuster tool definition`
