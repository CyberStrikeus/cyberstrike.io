...
function globalFiles() {
  ...
  if (!Flag.CYBERSTRIKE_DISABLE_CLAUDE_CODE_PROMPT) {
    files.push(path.join(os.homedir(), ".claude", "CLAUDE.md"))
  }
  if (Flag.CYBERSTRIKE_CONFIG_DIR) {
    files.push(path.join(Flag.CYBERSTRIKE_CONFIG_DIR, "AGENTS.md"))
  }
  return files
}
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.CYBERSTRIKE_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no CYBERSTRIKE_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.CYBERSTRIKE_CONFIG_DIR, Flag.CYBERSTRIKE_CONFIG_DIR).catch(() => [])
}
