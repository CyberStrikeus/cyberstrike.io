...
import { createCyberstrikeClient, type CyberstrikeClient } from "@cyberstrike-io/sdk/v2"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    let message = [...args.message, ...(args["--"] || [])]
      .map((arg) => (arg.includes(" ") ? `"${arg.replaceAll("\\", "\\\\").replaceAll('"', '\\"')}"` : arg))
      .join(" ")
    ...
    const execute = async (sdk: CyberstrikeClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createCyberstrikeClient({ baseUrl: args.attach })
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.CYBERSTRIKE_AUTO_SHARE || args.share)) {
