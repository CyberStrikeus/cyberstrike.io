...
const TIPS = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at cyberstrike.ai",
  ...
  "Create {highlight}cyberstrike.json{/highlight} in project root for project-specific settings",
  "Place settings in {highlight}~/.config/cyberstrike/cyberstrike.json{/highlight} for global config",
  ...
  "cyberstrike auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.cyberstrike/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.cyberstrike/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "cyberstrike auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "cyberstrike uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.cyberstrike/tool/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.cyberstrike/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent cyberstrike from reading sensitive files",
  "Use {highlight}cyberstrike run{/highlight} for non-interactive scripting",
  "Use {highlight}cyberstrike --continue{/highlight} to resume the last session",
  "Use {highlight}cyberstrike run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}cyberstrike serve{/highlight} for headless API access to cyberstrike",
  "Use {highlight}cyberstrike run --attach{/highlight} to connect to a running server",
  "Run {highlight}cyberstrike upgrade{/highlight} to update to the latest version",
  "Run {highlight}cyberstrike auth list{/highlight} to see all configured providers",
  "Run {highlight}cyberstrike agent create{/highlight} for guided agent creation",
  "Use {highlight}/cyberstrike{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}cyberstrike github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/cyberstrike fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.cyberstrike/themes/{/highlight} directory",
  ...
  "Run {highlight}cyberstrike debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/cyberstrike/cyberstrike{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with Cyberstrike Arsenal for curated, tested models",
