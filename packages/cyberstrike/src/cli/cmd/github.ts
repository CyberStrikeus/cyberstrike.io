...
const AGENT_USERNAME = "cyberstrike-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/cyberstrike.yml"
      ...
      async fn() {
        {
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/cyberstrike-agent"
            const command =
...
jobs:
  cyberstrike:
    if: |
      contains(github.event.comment.body, ' /wk') ||
      startsWith(github.event.comment.body, '/wk') ||
      contains(github.event.comment.body, ' /cyberstrike') ||
      startsWith(github.event.comment.body, '/cyberstrike')
    ...
    steps:
      ...
      - name: Run cyberstrike
        uses: cyberstrike/cyberstrike/github@latest${envStr}
        with:
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      try {
        ...
        console.log("cyberstrike session", session.id)
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/cyberstrike,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to cyberstrike...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("cyberstrike-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `cyberstrike/${type}-${hex}-${timestamp}`
        }
        return `cyberstrike/${type}${issueId}-${timestamp}`
      }
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the cyberstrike infrastructure after your response",
