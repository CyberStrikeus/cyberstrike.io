...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    if (Flag.CYBERSTRIKE_CONFIG) {
      result = mergeConfigConcatArrays(result, await loadFile(Flag.CYBERSTRIKE_CONFIG))
    ...
    if (Flag.CYBERSTRIKE_CONFIG_CONTENT) {
      result = mergeConfigConcatArrays(result, JSON.parse(Flag.CYBERSTRIKE_CONFIG_CONTENT))
    ...
    if (Flag.CYBERSTRIKE_DISABLE_AUTOCOMPACT) {
      result.compaction = { ...result.compaction, auto: false }
    }
    if (Flag.CYBERSTRIKE_DISABLE_PRUNE) {
  ...
  export async function installDependencies(dir: string) {
    ...
    await BunProc.run(
      ["add", "@cyberstrike-io/plugin@" + (Installation.isLocal() ? "latest" : Installation.VERSION), "--exact"],
      {
        cwd: dir,
      },
    ).catch(() => {})
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({
      ...
      const patterns = ["/.cyberstrike/command/", "/.cyberstrike/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for await (const item of AGENT_GLOB.scan({
      ...
      const patterns = ["/.cyberstrike/agent/", "/.cyberstrike/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
