...
test("loads agents from .whykido/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".opencode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .whykido/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".opencode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .whykido/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".opencode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "opencode.json"),
        JSON.stringify({ $schema: "https://whykido.dev/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .whykido config
      ...
      // Local .whykido config with different plugins
      await Bun.write(
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .whykido config with some overlapping plugins
      await Bun.write(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({
          $schema: "https://whykido.dev/config.json",
...
test("local .whykido config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.whykido/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.whykido/plugin/oh-my-opencode.js")
  })
...
describe("WHYKIDO_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["WHYKIDO_DISABLE_PROJECT_CONFIG"]
    process.env["WHYKIDO_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["WHYKIDO_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["WHYKIDO_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .whykido/ directories when flag is set", async () => {
    const originalEnv = process.env["WHYKIDO_DISABLE_PROJECT_CONFIG"]
    process.env["WHYKIDO_DISABLE_PROJECT_CONFIG"] = "true"
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["WHYKIDO_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["WHYKIDO_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["WHYKIDO_CONFIG_DIR"]
      process.env["WHYKIDO_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["WHYKIDO_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["WHYKIDO_CONFIG_DIR"]
      } else {
        process.env["WHYKIDO_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("WHYKIDO_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["WHYKIDO_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["WHYKIDO_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["WHYKIDO_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["WHYKIDO_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from WHYKIDO_CONFIG_DIR, not project
