...
test("plan agent denies edits except .whykido/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(PermissionNext.evaluate("edit", ".whykido/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
  ...
  await using tmp = await tmpdir({
    config: {
      agent: {
        build: { disable: true },
        plan: { disable: true },
        // Also disable pentest agents which are primary
        "web-application": { disable: true },
        "cloud-security": { disable: true },
        "internal-network": { disable: true },
        "bug-hunter": { disable: true },
      },
    },
  })
  await Instance.provide({
