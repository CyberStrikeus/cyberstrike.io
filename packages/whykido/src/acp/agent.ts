...
import type { Event, WhykidoClient, SessionMessageResponse } from "@whykido/sdk/v2"
import { applyPatch } from "diff"

export namespace ACP {
  const log = Log.create({ service: "acp-agent" })

  export async function init({ sdk: _sdk }: { sdk: WhykidoClient }) {
    return {
