      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://whykido.dev/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.whykido.dev/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  whykido:
    if: |
      ...
      contains(github.event.comment.body, ' /whykido') ||
      startsWith(github.event.comment.body, '/whykido')
    ...
    steps:
      ...
      - name: Run whykido
        uses: whykido/whykido/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.whykido.dev" : "https://whykido.dev"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.whykido.dev"
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `whykido/${type}-${hex}-${timestamp}`
        }
        return `whykido/${type}${issueId}-${timestamp}`
      }
