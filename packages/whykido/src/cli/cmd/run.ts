...
import { createWhykidoClient, type WhykidoClient } from "@whykido/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run whykido with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running whykido server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: WhykidoClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createWhykidoClient({ baseUrl: args.attach })
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.WHYKIDO_AUTO_SHARE || args.share)) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createWhykidoClient({ baseUrl: "http://opencode.internal", fetch: fetchFn })
