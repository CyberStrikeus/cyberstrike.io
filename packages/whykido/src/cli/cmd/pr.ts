...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run whykido",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for whykido session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found whykido session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`whykido import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting whykido...")
        UI.println()

        // Launch whykido TUI with session ID if available
        const { spawn } = await import("child_process")
        const whykidoArgs = sessionId ? ["-s", sessionId] : []
        const whykidoProcess = spawn("whykido", whykidoArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          whykidoProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`whykido exited with code ${code}`))
          })
          whykidoProcess.on("error", reject)
        })
      },
    })
  },
