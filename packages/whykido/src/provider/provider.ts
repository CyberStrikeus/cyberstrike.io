...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    openrouter: async () => {
      return {
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://whykido.dev/",
            "X-Title": "whykido",
          },
        },
      }
  ...
  export function fromModelsDevProvider(provider: ModelsDev.Provider): Info {
    // Rebrand OpenCode Zen to Whykido Zen
    const name = provider.name === "OpenCode Zen" ? "Whykido Zen" : provider.name
    return {
      id: provider.id,
      source: "custom",
      name,
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.WHYKIDO_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
