---
title: Server
description: Interact with whykido server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `whykido serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an whykido client can use.
...
```bash
whykido serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
whykido serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `WHYKIDO_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `whykido`, or set `WHYKIDO_SERVER_USERNAME` to override it. This applies to both `whykido serve` and `whykido web`.

```bash
WHYKIDO_SERVER_PASSWORD=your-password whykido serve
```
...
When you run `whykido` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the whykido server to interact with whykido programmatically.
:::

This architecture lets whykido support multiple clients and allows you to interact with whykido programmatically.

You can run `whykido serve` to start a standalone server. If you have the
whykido TUI running, `whykido serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Whykido [IDE](/docs/ide) plugins.
...
The whykido server exposes the following APIs.
