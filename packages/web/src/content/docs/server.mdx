---
title: Server
description: Interact with cyberstrike server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `cyberstrike serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an cyberstrike client can use.
...
Set `CYBERSTRIKE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `cyberstrike`, or set `CYBERSTRIKE_SERVER_USERNAME` to override it. This applies to both `cyberstrike serve` and `cyberstrike web`.

```bash
CYBERSTRIKE_SERVER_PASSWORD=your-password cyberstrike serve
```
...
When you run `cyberstrike` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the cyberstrike server to interact with cyberstrike programmatically.
:::

This architecture lets cyberstrike support multiple clients and allows you to interact with cyberstrike programmatically.

You can run `cyberstrike serve` to start a standalone server. If you have the
cyberstrike TUI running, `cyberstrike serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Cyberstrike [IDE](/docs/ide) plugins.
...
The cyberstrike server exposes the following APIs.
