---
title: Custom Tools
description: Create tools the LLM can call in whykido.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside whykido's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.whykido/tools/` directory of your project.
- Or globally, by placing them in `~/.config/whykido/tools/`.
...
```ts title=".whykido/tools/database.ts" {1}
...
```ts title=".whykido/tools/math.ts"
import { tool } from "@whykido/plugin"
...
```ts title=".whykido/tools/project.ts" {8}
import { tool } from "@whykido/plugin"
...
```python title=".whykido/tools/add.py"
import sys
...
```ts title=".whykido/tools/python-add.ts" {10}
import { tool } from "@whykido/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.directory, ".whykido/tools/add.py")
