---
title: Custom Tools
description: Create tools the LLM can call in cyberstrike.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside cyberstrike's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.cyberstrike/tools/` directory of your project.
- Or globally, by placing them in `~/.config/cyberstrike/tools/`.
...
```ts title=".cyberstrike/tools/database.ts" {1}
...
```ts title=".cyberstrike/tools/math.ts"
import { tool } from "@cyberstrike/plugin"
...
```ts title=".cyberstrike/tools/project.ts" {8}
import { tool } from "@cyberstrike/plugin"
...
```python title=".cyberstrike/tools/add.py"
import sys
...
```ts title=".cyberstrike/tools/python-add.ts" {10}
import { tool } from "@cyberstrike/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.directory, ".cyberstrike/tools/add.py")
