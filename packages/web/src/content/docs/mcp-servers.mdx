...
You can add external tools to Whykido using the _Model Context Protocol_, or MCP. Whykido supports both local and remote servers.
...
You can define MCP servers in your [Whykido Config](https://whykido.dev/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="whykido.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/whykido` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Whykido automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Whykido will:
...
If the server requires authentication, Whykido will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `whykido mcp auth <server-name>`.
...
```bash
whykido mcp auth my-oauth-server
```
...
```bash
whykido mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Whykido will store the tokens securely in `~/.local/share/whykido/mcp-auth.json`.
...
whykido mcp debug my-oauth-server
```
...
Your MCPs are available as tools in Whykido, alongside built-in tools. So you can manage them through the Whykido config like any other tool.
...
```json title="whykido.json" {11, 14-18}
{
  "$schema": "https://whykido.dev/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
whykido mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect Whykido to your Sentry account.
