---
title: SDK
description: Type-safe JS client for whykido server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The whykido JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control whykido programmatically.
...
Create an instance of whykido:

```javascript
import { createWhykido } from "@whykido/sdk"

const { client } = await createWhykido()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `whykido.json`, but you can override or add configuration inline:

```javascript
import { createWhykido } from "@whykido/sdk"

const whykido = await createWhykido({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${whykido.server.url}`)

whykido.server.close()
```

## Client only

If you already have a running instance of whykido, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@whykido/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*whykido" },
})
