---
title: SDK
description: Type-safe JS client for cyberstrike server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The cyberstrike JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control cyberstrike programmatically.
...
Create an instance of cyberstrike:

```javascript
import { createCyberstrike } from "@cyberstrike/sdk"

const { client } = await createCyberstrike()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `cyberstrike.json`, but you can override or add configuration inline:

```javascript
import { createCyberstrike } from "@cyberstrike/sdk"

const cyberstrike = await createCyberstrike({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${cyberstrike.server.url}`)
...
If you already have a running instance of cyberstrike, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@cyberstrike/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*cyberstrike" },
})
