...
With Cyberstrike you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Cyberstrike uses our own `cyberstrike` theme.
...
Cyberstrike comes with several built-in themes.
...
- Want Cyberstrike to match their terminal's appearance
...
Cyberstrike supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/cyberstrike/themes/*.json` or `$XDG_CONFIG_HOME/cyberstrike/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.cyberstrike/themes/*.json`
4. **Current working directory** - Defined in `./.cyberstrike/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/cyberstrike/themes
vim ~/.config/cyberstrike/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .cyberstrike/themes
vim .cyberstrike/themes/my-theme.json
```
...
{
  "$schema": "https://cyberstrike.io/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
