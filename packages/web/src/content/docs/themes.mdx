...
With Whykido you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Whykido uses our own `whykido` theme.
...
Whykido comes with several built-in themes.
...
- Want Whykido to match their terminal's appearance
...
Whykido supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/whykido/themes/*.json` or `$XDG_CONFIG_HOME/whykido/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.whykido/themes/*.json`
4. **Current working directory** - Defined in `./.whykido/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/whykido/themes
vim ~/.config/whykido/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .whykido/themes
vim .whykido/themes/my-theme.json
```
...
{
  "$schema": "https://whykido.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
