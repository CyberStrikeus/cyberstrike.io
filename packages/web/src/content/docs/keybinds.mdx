...
Whykido has a list of keybinds that you can customize through the Whykido config.
...
Whykido uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The Whykido desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `whykido.json`.
