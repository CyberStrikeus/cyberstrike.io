...
:::caution
Modes are now configured through the `agent` option in the whykido config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in whykido allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the whykido config.
...
whykido comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.whykido/plans/*.md` to detail the plan itself
...
Configure modes in your `whykido.json` config file:
...
- Global: `~/.config/whykido/modes/`
- Project: `.whykido/modes/`

```markdown title="~/.config/whykido/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global whykido config and the project specific config.
...
Create mode files in `.whykido/modes/` for project-specific modes or `~/.config/whykido/modes/` for global modes:

```markdown title=".whykido/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/whykido/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
