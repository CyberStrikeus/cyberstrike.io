---
title: LSP Servers
description: Cyberstrike integrates with your LSP servers.
---

Cyberstrike integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

Cyberstrike comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `CYBERSTRIKE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When cyberstrike opens a file, it:
...
You can customize LSP servers through the `lsp` section in your cyberstrike config.
