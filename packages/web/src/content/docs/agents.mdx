...
There are two types of agents in Cyberstrike; primary agents and subagents.
...
Cyberstrike comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Cyberstrike comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Cyberstrike comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `cyberstrike.json` config file:
...
- Global: `~/.config/cyberstrike/agents/`
- Per-project: `.cyberstrike/agents/`

```markdown title="~/.config/cyberstrike/agents/review.md"
---
...
If no temperature is specified, Cyberstrike uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Cyberstrike config and the project specific config.
...
The model ID in your Cyberstrike config uses the format `provider/model-id`. For example, if you're using [Cyberstrike Arsenal](/docs/arsenal), you would use `cyberstrike/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="cyberstrike.json" {3-6,9-12}
{
  "$schema": "https://cyberstrike.io/config.json",
  "tools": {
    "write": true,
...
```json title="cyberstrike.json" {3-5,8-10}
{
  "$schema": "https://cyberstrike.io/config.json",
  "permission": {
    "edit": "deny"
...
```json title="cyberstrike.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Run `cyberstrike models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/CyberStrikeus/cyberstrike.io).
:::
...
```markdown title="~/.config/cyberstrike/agents/docs-writer.md"
---
...
```markdown title="~/.config/cyberstrike/agents/security-auditor.md"
---
