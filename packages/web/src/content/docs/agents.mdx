...
There are two types of agents in Whykido; primary agents and subagents.
...
Whykido comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Whykido comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Whykido comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `whykido.json` config file:
...
- Global: `~/.config/whykido/agents/`
- Per-project: `.whykido/agents/`

```markdown title="~/.config/whykido/agents/review.md"
---
...
If no temperature is specified, Whykido uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Whykido config and the project specific config.
...
The model ID in your Whykido config uses the format `provider/model-id`. For example, if you're using [Whykido Zen](/docs/zen), you would use `whykido/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="whykido.json" {3-6,9-12}
{
  "$schema": "https://whykido.dev/config.json",
  "tools": {
    "write": true,
...
```json title="whykido.json" {3-5,8-10}
{
  "$schema": "https://whykido.dev/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `whykido models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/whykido/whykido).
:::
...
```markdown title="~/.config/whykido/agents/docs-writer.md"
---
...
```markdown title="~/.config/whykido/agents/security-auditor.md"
---
