...
Tools allow the LLM to perform actions in your codebase. Cyberstrike comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Cyberstrike.
...
:::note
This tool is only available when `CYBERSTRIKE_EXPERIMENTAL_LSP_TOOL=true` (or `CYBERSTRIKE_EXPERIMENTAL=true`).
:::
