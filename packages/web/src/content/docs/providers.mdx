---
title: Providers
description: Using any LLM provider in Cyberstrike.
---

import config from "../../../config.mjs"
export const console = config.console

Cyberstrike uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Cyberstrike config.
...
in `~/.local/share/cyberstrike/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Cyberstrike
config.
...
Cyberstrike Arsenal is a list of models provided by the Cyberstrike team that have been
tested and verified to work well with Cyberstrike. [Learn more](/docs/arsenal).

:::tip
If you are new, we recommend starting with Cyberstrike Arsenal.
:::

1. Run the `/connect` command in the TUI, select cyberstrike, and head to [cyberstrike.io/auth](https://cyberstrike.io/auth).
...
It works like any other provider in Cyberstrike and is completely optional to use.
...
To use Amazon Bedrock with Cyberstrike:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running cyberstrike:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY cyberstrike

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile cyberstrike

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX cyberstrike
   ```
   ...
   For project-specific or persistent configuration, use `cyberstrike.json`:
...
:::info
Using your Claude Pro/Max subscription in Cyberstrike is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for cyberstrike to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX cyberstrike
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX cyberstrike
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your cyberstrike config.
...
to store token in cyberstrike auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Cyberstrike uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Cyberstrike
to only use your own GitLab-hosted instance, add the following to your
`cyberstrike.json` file. It is also recommended to disable session sharing.
...
More documentation on [cyberstrike-gitlab-auth](https://www.npmjs.com/package/@gitlab/cyberstrike-gitlab-auth) homepage.

##### Configuration

Customize through `cyberstrike.json`:
...
{
  "$schema": "https://cyberstrike.io/config.json",
  "plugin": ["@gitlab/cyberstrike-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with cyberstrike:
...
To use Google Vertex AI with Cyberstrike:
...
2. Set the required environment variables:
   ...
   Set them while running cyberstrike.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id cyberstrike
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through cyberstrike, you can always configure it yourself.
...
```jsonc title="~/.config/cyberstrike.jsonc"
{
  "$schema": "https://cyberstrike.io/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [cyberstrike-helicone-session](https://github.com/H2Shami/cyberstrike-helicone-session) plugin to automatically log each Cyberstrike conversation as a session in Helicone.

```bash
npm install -g cyberstrike-helicone-session
```
...
```json title="cyberstrike.json"
{
  "plugin": ["cyberstrike-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Cyberstrike conversation listed as a separate session.
...
You can configure cyberstrike to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="cyberstrike.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://cyberstrike.io/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure cyberstrike to use local models through LM Studio.

```json title="cyberstrike.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://cyberstrike.io/config.json",
  "provider": {
    "lmstudio": {
...
You can configure cyberstrike to use local models through Ollama.

```json title="cyberstrike.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://cyberstrike.io/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Cyberstrike:
...
3. Copy the API key for use in Cyberstrike.
...
6. **Important**: Before using cloud models in Cyberstrike, you must pull the model information locally:
...
Cyberstrike Arsenal is a list of tested and verified models provided by the Cyberstrike team. [Learn more](/docs/arsenal).

1. Sign in to **<a href={console}>Cyberstrike Arsenal</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Cyberstrike Arsenal**.

   ```txt
   /connect
   ```

3. Enter your Cyberstrike API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your cyberstrike config.
...
5. You can also customize them through your cyberstrike config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' cyberstrike
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group cyberstrike
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Cyberstrike:
...
You can also customize models through your cyberstrike config. Here's an example of specifying provider routing order.
...
1. Head over to the [ZenMux dashboard](https://arsenalmux.ai/settings/keys), click **Create API Key**, and copy the key.
...
:::tip
You can use any OpenAI-compatible provider with cyberstrike. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in cyberstrike.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `cyberstrike.json` file in your project directory:

   ```json title="cyberstrike.json" ""myprovider"" {5-15}
   {
     "$schema": "https://cyberstrike.io/config.json",
     "provider": {
       "myprovider": {
...
```json title="cyberstrike.json" {9,11,17-20}
{
  "$schema": "https://cyberstrike.io/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Cyberstrike to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `cyberstrike auth list` to see if the credentials
...
2. For custom providers, check the cyberstrike config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your cyberstrike config.
