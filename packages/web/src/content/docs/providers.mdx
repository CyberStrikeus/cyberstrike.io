---
title: Providers
description: Using any LLM provider in Whykido.
---

import config from "../../../config.mjs"
export const console = config.console

Whykido uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Whykido config.
...
in `~/.local/share/whykido/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Whykido
config.
...
Whykido Zen is a list of models provided by the Whykido team that have been
tested and verified to work well with Whykido. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Whykido Zen.
:::

1. Run the `/connect` command in the TUI, select whykido, and head to [whykido.dev/auth](https://whykido.dev/auth).
...
It works like any other provider in Whykido and is completely optional to use.
...
To use Amazon Bedrock with Whykido:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running whykido:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY whykido

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile whykido

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX whykido
   ```
   ...
   For project-specific or persistent configuration, use `whykido.json`:

   ```json title="whykido.json"
   {
     "$schema": "https://whykido.dev/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Using your Claude Pro/Max subscription in Whykido is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for whykido to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX whykido
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX whykido
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your whykido config.
...
to store token in whykido auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Whykido uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Whykido
to only use your own GitLab-hosted instance, add the following to your
`whykido.json` file. It is also recommended to disable session sharing.
...
Customize through `whykido.json`:
...
To use your GitHub Copilot subscription with whykido:
...
To use Google Vertex AI with Whykido:
...
2. Set the required environment variables:
   ...
   Set them while running whykido.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id whykido
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through whykido, you can always configure it yourself.
...
```jsonc title="~/.config/whykido.jsonc"
{
  "$schema": "https://whykido.dev/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Whykido conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Whykido conversation listed as a separate session.
...
You can configure whykido to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="whykido.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://whykido.dev/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure whykido to use local models through LM Studio.

```json title="whykido.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://whykido.dev/config.json",
  "provider": {
    "lmstudio": {
...
You can configure whykido to use local models through Ollama.

```json title="whykido.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://whykido.dev/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Whykido:
...
3. Copy the API key for use in Whykido.
...
6. **Important**: Before using cloud models in Whykido, you must pull the model information locally:
...
Whykido Zen is a list of tested and verified models provided by the Whykido team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Whykido Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Whykido Zen**.

   ```txt
   /connect
   ```

3. Enter your Whykido API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your whykido config.
...
5. You can also customize them through your whykido config. Here's an example of specifying a provider
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group whykido
   ```
...
You can also customize models through your whykido config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with whykido. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in whykido.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `whykido.json` file in your project directory:

   ```json title="whykido.json" ""myprovider"" {5-15}
   {
     "$schema": "https://whykido.dev/config.json",
     "provider": {
       "myprovider": {
...
```json title="whykido.json" {9,11,17-20}
{
  "$schema": "https://whykido.dev/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Whykido to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `whykido auth list` to see if the credentials
...
2. For custom providers, check the whykido config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your whykido config.
