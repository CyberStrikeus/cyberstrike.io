...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start Whykido.
...
Here are several models that work well with Whykido, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [Whykido Zen](/docs/zen), you would use `whykido/gpt-5.1-codex` for GPT 5.1 Codex.
...
```jsonc title="whykido.jsonc" {7-12,19-24}
{
  "$schema": "https://whykido.dev/config.json",
  "provider": {
    "openai": {
...
```jsonc title="whykido.jsonc" {6-21}
{
  "$schema": "https://whykido.dev/config.json",
  "provider": {
    "whykido": {
...
Many models support multiple variants with different configurations. Whykido ships with built-in default variants for popular providers.

### Built-in variants

Whykido ships with default variants for many providers:
...
```jsonc title="whykido.jsonc" {7-18}
{
  "$schema": "https://whykido.dev/config.json",
  "provider": {
    "openai": {
...
When Whykido starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the Whykido config.
