...
import { Tag } from "@cyberstrike-io/ui/tag"
...
export const SettingsProviders: Component = () => {
  ...
  const connected = createMemo(() => {
    return providers
      .connected()
      .filter((p) => p.id !== "cyberstrike" || Object.values(p.models).find((m) => m.cost?.input))
  })
                ...
                <div class="flex items-center justify-between gap-4 h-16 border-b border-border-weak-base last:border-none">
                  <div class="flex flex-col min-w-0">
                    <div class="flex items-center gap-x-3">
                      ...
                      <Show when={item.id === "cyberstrike"}>
                        <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
                      </Show>
                    </div>
                    <Show when={item.id === "cyberstrike"}>
                      <span class="text-12-regular text-text-weak pl-8">
                        {language.t("dialog.provider.cyberstrike.note")}
                      </span>
                    </Show>
