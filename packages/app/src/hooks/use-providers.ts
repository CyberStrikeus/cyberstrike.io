...
export const popularProviders = ["cyberstrike", "anthropic", "github-copilot", "openai", "google", "openrouter", "vercel"]
...
export function useProviders() {
  ...
  const paid = createMemo(() =>
    connected().filter((p) => p.id !== "cyberstrike" || Object.values(p.models).find((m) => m.cost?.input)),
  )
